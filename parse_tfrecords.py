...
def _parse_tfrecord(example_proto):
    ...
    img_data = tf.decode_raw(parsed_features['img_data'], tf.uint8)
    ...
    with tf.device('/cpu:0'):
        ...
        if is_training:
            ...
            dataset = (dataset.map(flip_image, num_parallel_calls=num_threads)
                       ...
                       .map(lambda x: resize_image(*x, image_size=image_size), num_parallel_calls=num_threads))
            dataset = dataset.batch(batch_size, drop_remainder=True)
            dataset = dataset.prefetch(1)  # 取出一个batch
        else:
            dataset = dataset.map(resize_image, num_parallel_calls=num_threads)
